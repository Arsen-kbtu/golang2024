...
// key.
func (app *application) contextSetUser(r *http.Request, user *pkg.User) *http.Request {
...
func (app *application) contextGetUser(r *http.Request) *pkg.User {
	user, ok := r.Context().Value(userContextKey).(*pkg.User)
	if !ok {
