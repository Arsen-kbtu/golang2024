...
func (app *application) createAuthenticationTokenHandler(w http.ResponseWriter, r *http.Request) {
	...
	v := validator.New()
	pkg.ValidateEmail(v, input.Email)
	pkg.ValidatePasswordPlaintext(v, input.Password)
	if !v.Valid() {
	...
	if err != nil {
		app.serverErrorResponse(w, r, err)
		fmt.Sprintf("passwords don't match")
		return
	}
	...
	token, err := app.models.Tokens.New(user.ID, 24*time.Hour, pkg.ScopeAuthentication)
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}
